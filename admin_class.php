...
function delete_product(){
	...
	function chk_prod_availability(){
		...
		$available = $inn - $out;
		return json_encode(array('available'=>$available));
	...
	if(empty($id)){
			...
			if(empty($id)){
				...
				foreach($product_id as $k => $v){
					...
					$data .= ", recommended = '$recommended[$k]' ";
					$data .= ", approved = '$approved[$k]' ";
					$data .= ", issued = '$issued[$k]' ";
					$data .= ", status = 'Pending' ";
					$save2[]= $this->db->query("INSERT INTO inventory set ".$data);
				}
				if(isset($save2)){
	...
	function update_status(){
				if(!empty($id)){
				$form_id = $this->db->query("SELECT * FROM sales_list where id =$id ");
				// $id =$this->db->insert_id;
				foreach($product_id as $k => $v){
